...
namespace GUI
{
    public partial class FormPrincipal : Form
    {
        ...
        private void asignarBusRutasToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form frm = this.MdiChildren.OfType<Form>().Where(x => x.Name.Equals("FormBusesRutas")).FirstOrDefault();

            if (frm == null)
            {
                Gestion.FormBusesRutas formBusesRutas = new Gestion.FormBusesRutas();
                formBusesRutas.MdiParent = this;
                formBusesRutas.Show();
            }
        }
        ...
        {
            Form frm = this.MdiChildren.OfType<Form>().Where(x => x.Name.Equals("FormVentas")).FirstOrDefault();

            if (frm == null)
            {
                Ventas.FormVentas formVentas = new Ventas.FormVentas();
