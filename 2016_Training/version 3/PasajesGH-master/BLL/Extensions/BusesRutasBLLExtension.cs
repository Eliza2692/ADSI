...
namespace BLL.Extensions
{
    public static class BusesRutasBLLExtension
    {
        ...
        static public IEnumerable<object> GetAllHorario(this BusesRutasBLL r, int IdOrigen, int IdDestino)
        {
            try
            {
                using (db = new PasajesBDEntities())
                {
                    return (from t in db.HORARIOS
                            ...
                            join ciu in db.CIUDADES
                            on rut.CiudadDestino equals ciu.IdCiudad
                            where rut.CiudadOrigen == IdOrigen && rut.CiudadDestino == IdDestino
                            group ciu by new {t.Hora, t.IdHorario} into g
