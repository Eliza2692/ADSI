...
-- --------------------------------------------------
-- Date Created: 09/21/2016 09:41:42
-- Generated from EDMX file: C:\Users\TECNOPARQUE\Desktop\PasajesGH-master\DAL\Model\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_BUSES_RUTAS_RUTAS]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[BUSES_RUTAS] DROP CONSTRAINT [FK_BUSES_RUTAS_RUTAS];
GO
...
-- Creating foreign key on [IdRuta] in table 'BUSES_RUTAS'
ALTER TABLE [dbo].[BUSES_RUTAS]
ADD CONSTRAINT [FK_BUSES_RUTAS_RUTAS]
    FOREIGN KEY ([IdRuta])
    REFERENCES [dbo].[RUTAS]
        ([IdRuta])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_BUSES_RUTAS_RUTAS'
CREATE INDEX [IX_FK_BUSES_RUTAS_RUTAS]
ON [dbo].[BUSES_RUTAS]
    ([IdRuta]);
GO
