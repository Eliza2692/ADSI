...
namespace BLL.Extensions
{
    public static class DetalleFacturaBLLExtension
    {
            ...
            try
            {
                using (db = new PasajesBDEntities())
                {
                    return (from t in db.BUSES_RUTAS
                            ...
                            group ciu by new { hor.Hora, t.IdHorario, t.idBus, t.IdBusRuta } into g
                            select new { g.Key.Hora, g.Key.IdHorario, g.Key.idBus, g.Key.IdBusRuta }).ToList();
                }
            }
            catch (Exception)
            {
        ...
        static public List<PUESTOS> GetAllPuestos(this DetalleFacturaBLL r, int idBusRuta)
        {
            try
            {
                using (db = new PasajesBDEntities())
        ...
        static public int GetLastIdFactura(this DetalleFacturaBLL r)
        {
            try
            {
                using (db = new PasajesBDEntities())
                {
                    var i = (from t in db.FACTURAS
                            orderby t.IdFactura descending
                            select t);

                    return i.First().IdFactura;
                }
            }
            catch (Exception)
            {
